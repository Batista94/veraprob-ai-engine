...
Structural Sharding: O sistema divide o documento com base em cabeçalhos Markdown (## Cláusula 4). Apenas as seções classificadas previamente como "Relevantes para SLA" são enviadas para a etapa de extração profunda.
...
Structural Sharding: The system partitions the document based on Markdown headers (## Clause 4). Only sections pre-classified as "SLA-Relevant" are forwarded to the deep extraction node.
